...
def main():
    ...
    parser.add_argument('--seed', type=Path,
                        default=ROOT / 'sylver/campaigns/w-seven-2026-09-22/deep/proof-graph.json')
    ...
    for name in ('b76-even', 'b76-reply36-46'):
        path = ROOT / 'sylver/campaigns/targeted-2026-09-22' / name / 'proof-graph.json'
    ...
    checkpoint()
    # A STOP file requests a clean exit after the current complete checkpoint.
    while elapsed < args.budget and not found and not (out / 'STOP').exists():
        queues = {}
        for m in MOVES:
            if graph.route((*W, m)):
                continue
