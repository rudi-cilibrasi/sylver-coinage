...
struct Engine {
    ...
    std::set<int> cached_single_anchors;
    ...
    bool ensure_reset_history(int eid, int cutoff) {
        ...
        int sid = single_sid[eid];
        if (sid < 0) sid = register_shape(eid, {0});
        // Reset is existential. A known later P in the required prefix
        // makes it unnecessary to solve an earlier cache miss. Candidate
        // anchors alone are merely hints: the exact canonical E+anchor key
        // must itself be present as P before the reset is accepted.
        for (const int anchor : cached_single_anchors) {
            if (anchor > cutoff) break;
            const auto known = cached_outcome(eid, {anchor});
            if (known.has_value() && *known) {
                evaluate(sid, anchor);  // Records the exact P and first_p.
                return true;
            }
        }
...
int main(int argc, char** argv) {
    ...
    for (int i = 3; i < argc; ++i) {
        ...
        } else if (argument == "--target-start") {
            if (++i >= argc) {
                std::cerr << "--target-start requires an odd starting move\n";
                return 1;
            }
            try {
                std::size_t parsed = 0;
                const std::string text = argv[i];
                target_start = std::stol(text, &parsed);
                if (parsed != text.size() || target_start < 3 || target_start % 2 == 0
                    || target_start > std::numeric_limits<int>::max())
                    throw std::invalid_argument("target start");
            } catch (const std::exception&) {
                std::cerr << "invalid --target-start move\n";
                return 1;
            }
        } else if (argument == "--stop-on-p") {
            stop_on_p = true;
    ...
    if ((target_start != 0 || stop_on_p) && !eng.target_only) {
        std::cerr << "target range options require --target-only\n";
        return 1;
    }
    if (eng.target_only) {
        try {
    ...
    if (eng.target_only && target_start > limit) {
        std::cerr << "target start exceeds target limit\n";
        return 1;
    }
    ...
    if (eng.target_only) {
        ...
        for (const auto& [key, is_p] : eng.base_cache) {
            if (!is_p) continue;
            std::istringstream generators(key);
            std::string token;
            ...
            while (std::getline(generators, token, ',')) {
                ...
                if (value % 2 != 0) { ++count; anchor = value; }
            }
            if (count == 1) eng.cached_single_anchors.insert(anchor);
        }
        eng.begin_row();
        long target = target_start == 0 ? limit : target_start;
        try {
          for (; target <= limit; target += 2) {
            for (;;) {
                ...
                if (result >= 0) {
                    const auto gens = eng.position_generators(0, {static_cast<int>(target)});
                    eng.base_cache[Engine::generator_key(gens)] = result == 1;
                    eng.dirty_cache = true;
                    eng.save_cache();
                    std::cout << "TARGET n=" << target
          ...
          return 0;
            ...
            eng.save_cache();
            std::cout << "TARGET n=" << target << " outcome=unknown"
                      << " shapes=" << eng.shapes.size()
                      << " evaluations=" << eng.evaluation_calls
